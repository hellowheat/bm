...
namespace UnityChan
{
	public class AutoBlink : MonoBehaviour
	{
		...
		public bool isActive = true;                //オート目パチ有効,自动眼珠贴图有效
		public SkinnedMeshRenderer ref_SMR_EYE_DEF; //EYE_DEFへの参照，参考EYE DEF
		...
		public float timeBlink = 0.4f;              //目パチの時間,眨眼的时间
		private float timeRemining = 0.0f;          //タイマー残り時間，计时器剩余时间

		public float threshold = 0.3f;              // ランダム判定の閾値,随机判断的阈值
		public float interval = 3.0f;               // ランダム判定のインターバル，随机判定间隔
		...
		private Status eyeStatus;   //現在の目パチステータス，当前的眼睛状态
		...
		{
			ResetTimer ();
			// ランダム判定用関数をスタートする，启动RandomChange函数
